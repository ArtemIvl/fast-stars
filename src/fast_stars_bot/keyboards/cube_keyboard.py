...
from utils.cube_requests import get_active_count, get_waiting_count
...
async def cube_keyboard() -> InlineKeyboardMarkup:
    ...
    async with SessionLocal() as session:
        for bet in (Decimal("1"), Decimal("5"), Decimal("10")):
            ...
            rows.append(
                [InlineKeyboardButton(text=text, callback_data=f"cube_bet_{bet}")]
            )

    rows.append(
        [InlineKeyboardButton(text="🔄 Обновить", callback_data="cube_refresh")]
    )
    rows.append([InlineKeyboardButton(text="🔙 Назад", callback_data="games")])
...
def back_to_cube_menu_keyboard() -> InlineKeyboardMarkup:
    inline_keyboard = [
        [InlineKeyboardButton(text="🔙 Вернуться к столам", callback_data="cube_game")]
    ]
    return InlineKeyboardMarkup(inline_keyboard=inline_keyboard)
...
def end_game_keyboard(bet: Decimal) -> InlineKeyboardMarkup:
    inline_keyboard = [
        [InlineKeyboardButton(text="🔁 Сыграть ещё", callback_data=f"cube_bet_{bet}")],
        [InlineKeyboardButton(text="🔙 Выйти", callback_data="cube_game")],
    ]
    return InlineKeyboardMarkup(inline_keyboard=inline_keyboard)
