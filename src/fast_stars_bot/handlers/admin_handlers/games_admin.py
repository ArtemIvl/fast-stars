...
from keyboards.admin_keyboards import manage_games_keyboard, back_to_games_keyboard
...
async def manage_games_callback(callback: types.CallbackQuery, state: FSMContext) -> None:
    await state.clear()
    async with SessionLocal() as session:
        ...
        await callback.message.edit_text(text, parse_mode="HTML", reply_markup=manage_games_keyboard())

@games_admin_router.callback_query(F.data.in_({"change_x2game", "change_basketball", "change_cube"}))
async def start_setting_change(callback: types.CallbackQuery, state: FSMContext):
    ...
    sent = await callback.message.edit_text(f"Введите новое значение для: <b>{field_names[key]}</b>", parse_mode="HTML", reply_markup=back_to_games_keyboard())
...
async def set_setting_value(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if value <= 0 or value > 100:
            sent = await message.answer("Число должно быть больше 0 и не больше 100. Попробуйте снова.", reply_markup=back_to_games_keyboard())
            await state.update_data(last_bot_message_id=sent.message_id)
            return
    except InvalidOperation:
        sent = await message.answer("Введите корректное число.", reply_markup=back_to_games_keyboard())
        await state.update_data(last_bot_message_id=sent.message_id)
        return
    ...
    await message.answer("✅ Настройка успешно обновлена.", reply_markup=back_to_games_keyboard())
    await state.clear()