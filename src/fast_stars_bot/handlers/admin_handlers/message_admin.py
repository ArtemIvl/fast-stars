...
from keyboards.admin_keyboards import audience_keyboard, time_choice_keyboard, admin_back_keyboard, back_to_message_keyboard
...
async def choose_audience(callback: types.CallbackQuery, state: FSMContext):
    ...
    sent = await callback.message.edit_text("Введите сообщение для рассылки (поддерживается форматирование Telegram):", reply_markup=back_to_message_keyboard())
...
async def receive_message_text(message: types.Message, state: FSMContext):
    ...
    sent = await message.answer("Когда отправить сообщение? Введите время в формате ЧЧ:ММ или выберите:", reply_markup=time_choice_keyboard())
    await state.update_data(last_bot_message_id=sent.message_id)

@message_admin_router.callback_query(F.data == "send_now")
async def send_now(callback: types.CallbackQuery, state: FSMContext):
    data = await state.get_data()
    sent = await callback.message.edit_text("Отправка запущена...", reply_markup=admin_back_keyboard())
    await send_broadcast(callback.bot, data)
    try:
        await callback.bot.delete_message(callback.message.chat.id, sent.message_id)
    except Exception:
        pass
    await callback.message.answer("Рассылка завершена.", reply_markup=admin_back_keyboard())
    await state.clear()
...
async def schedule_send(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        sent = await message.answer("Неверный формат времени. Введите в формате ЧЧ:ММ", reply_markup=back_to_message_keyboard())
        await state.update_data(last_bot_message_id=sent.message_id)