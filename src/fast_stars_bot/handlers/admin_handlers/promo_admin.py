...
from keyboards.admin_keyboards import (
    back_to_promo_keyboard,
    manage_promo_keyboard,
    promo_pages_keyboard,
)
from utils.promo_requests import (
    add_promo_code,
    get_promo_activations_page,
    get_promo_code,
    is_promo_code_unique,
)
...
async def manage_promocodes_callback(
...
async def add_promo_callback(callback: types.CallbackQuery, state: FSMContext) -> None:
    sent = await callback.message.edit_text(
        "Введите промокод:", reply_markup=back_to_promo_keyboard()
    )
...
async def add_promo_name(message: types.Message, state: FSMContext) -> None:
    ...
    async with SessionLocal() as session:
        if not await is_promo_code_unique(session, code):
            sent = await message.answer(
                "Промокод уже существует. Попробуйте другой.",
                reply_markup=back_to_promo_keyboard(),
            )
            await state.update_data(last_bot_message_id=sent.message_id)
            return
    ...
    sent = await message.answer(
        "Введите количество активаций:", reply_markup=back_to_promo_keyboard()
    )
    await state.update_data(last_bot_message_id=sent.message_id)
...
async def add_promo_activations(message: types.Message, state: FSMContext) -> None:
    ...
    try:
        ...
        if activations <= 0:
            sent = await message.answer(
                "Количество активаций должно быть больше 0. Попробуйте снова.",
                reply_markup=back_to_promo_keyboard(),
            )
            await state.update_data(last_bot_message_id=sent.message_id)
            return
        ...
        sent = await message.answer(
            "Количество активаций должно быть целым числом. Попробуйте снова.",
            reply_markup=back_to_promo_keyboard(),
        )
        await state.update_data(last_bot_message_id=sent.message_id)
        return
...
async def add_promo_vip_state(message: types.Message, state: FSMContext) -> None:
    ...
    try:
        ...
        return
    sent = await message.answer(
        "Введите награду за активацию (⭐️):", reply_markup=back_to_promo_keyboard()
    )
    await state.update_data(last_bot_message_id=sent.message_id)
...
async def add_promo_reward(message: types.Message, state: FSMContext) -> None:
    ...
    try:
        ...
        sent = await message.answer(
            "Награда должна быть числом. Попробуйте снова.",
            reply_markup=back_to_promo_keyboard(),
        )
        await state.update_data(last_bot_message_id=sent.message_id)
        return
    ...
    await message.answer(
        f"Промокод {promo_code} успешно добавлен с {max_activations} активациями и наградой {reward}⭐️.",
        reply_markup=back_to_promo_keyboard(),
    )
    await state.clear()
...
async def view_promo_callback(callback: types.CallbackQuery, state: FSMContext) -> None:
    sent = await callback.message.edit_text(
        "Введите название промокода, информацию о котором вы хотите посмотреть:",
        reply_markup=back_to_promo_keyboard(),
    )
    await state.update_data(last_bot_message_id=sent.message_id)
...
def generate_promo_info_text(
    promo: PromoCode,
    activations: list[PromoActivation],
    page: int = 1,
    per_page: int = 10,
...
async def process_promo_query(message: types.Message, state: FSMContext) -> None:
    ...
    async with SessionLocal() as session:
        ...
        if not promo:
            sent = await message.answer(
                "Промокод не найден.", reply_markup=back_to_promo_keyboard()
            )
            await state.update_data(last_bot_message_id=sent.message_id)
            return
