...
from utils.giveaway_requests import get_all_giveaways, get_giveaway_by_id, get_tickets_for_giveaway, create_giveaway, delete_giveaway
from keyboards.admin_keyboards import back_to_giveaways_keyboard, manage_giveaways_keyboard, giveaway_info_keyboard
from pytz import timezone
from decimal import Decimal, InvalidOperation
from datetime import datetime, timedelta
from services.giveaway_scheduler import schedule_giveaway_finish, unschedule_giveaway_finish
...
async def manage_giveaways_callback(callback: types.CallbackQuery, state: FSMContext) -> None:
    await state.clear()
...
async def info_giveaway_handler(callback: types.CallbackQuery) -> None:
    ...
    await callback.message.edit_text(
        ...
        f"Розыгрыш от админа? <b>{f'Да, {giveaway.channel_link}' if giveaway.channel_link else 'Нет'}</b>\n\n"
...
async def add_giveaway_callback(callback: types.CallbackQuery, state: FSMContext) -> None:
    sent = await callback.message.edit_text("Введите название розыгрыша:", reply_markup=back_to_giveaways_keyboard())
...
async def set_giveaway_name(message: types.Message, state: FSMContext):
    ...
    sent = await message.answer("Введите ссылку на канал:", reply_markup=back_to_giveaways_keyboard())
    await state.update_data(last_bot_message_id=sent.message_id)
...
async def set_giveaway_channel_link(message: types.Message, state: FSMContext) -> None:
    ...
    sent = await message.answer("Введите призовой фонд (в звёздах):", reply_markup=back_to_giveaways_keyboard())
    await state.update_data(last_bot_message_id=sent.message_id)
...
async def set_giveaway_prize_pool(message: types.Message, state: FSMContext) -> None:
    ...
    try:
        ...
        if prize <= 0:
            sent = await message.answer("Вознаграждение должно быть больше 0. Попробуйте снова.", reply_markup=back_to_giveaways_keyboard())
            await state.update_data(last_bot_message_id=sent.message_id)
            return
        await state.update_data(prize_pool=prize)
        sent = await message.answer("Введите количество победителей:", reply_markup=back_to_giveaways_keyboard())
        await state.update_data(last_bot_message_id=sent.message_id)
        await state.set_state(AddGiveawayState.waiting_for_num_winners)
    except InvalidOperation:
        sent = await message.answer("Некорректный ввод. Пожалуйста, введите число.", reply_markup=back_to_giveaways_keyboard())
        await state.update_data(last_bot_message_id=sent.message_id)
        return
...
async def set_giveaway_num_winners(message: types.Message, state: FSMContext) -> None:
    ...
    try:
        ...
        if winners <= 0:
            sent = await message.answer("Количество победителей должно быть больше 0. Попробуйте снова.", reply_markup=back_to_giveaways_keyboard())
            await state.update_data(last_bot_message_id=sent.message_id)
            return
        await state.update_data(num_of_winners=winners)
        sent = await message.answer("Введите дату и время начала розыгрыша (в формате ДД.ММ.ГГГГ / ЧЧ:ММ) по Киеву:", reply_markup=back_to_giveaways_keyboard())
        await state.update_data(last_bot_message_id=sent.message_id)
        await state.set_state(AddGiveawayState.waiting_for_start_time)
    except ValueError:
        sent = await message.answer("Количество победителей должно быть целым числом. Попробуйте снова.", reply_markup=back_to_giveaways_keyboard())
        await state.update_data(last_bot_message_id=sent.message_id)
        return
...
async def set_giveaway_start_time(message: types.Message, state: FSMContext) -> None:
    ...
    try:
        ...
        start_dt = kyiv_tz.localize(datetime.strptime(date_str.strip() + " " + time_str.strip(), "%d.%m.%Y %H:%M"))

        if start_dt <= datetime.now(kyiv_tz):
            sent = await message.answer("⛔ Дата начала должна быть в будущем. Попробуйте снова.", reply_markup=back_to_giveaways_keyboard())
            await state.update_data(last_bot_message_id=sent.message_id)
            return
        await state.update_data(start_time=start_dt)
        sent = await message.answer("⏳ Введите длительность розыгрыша в формате: <b>дней часов минут</b>\nПример: <code>3 5 30</code>", parse_mode="HTML", reply_markup=back_to_giveaways_keyboard())
        await state.update_data(last_bot_message_id=sent.message_id)
        await state.set_state(AddGiveawayState.waiting_for_duration)
    except Exception as e:
        print(str(e))
        sent = await message.answer("❗Неверный формат. Введите дату в формате: <code>ДД.ММ.ГГГГ / ЧЧ:ММ</code>", parse_mode="HTML", reply_markup=back_to_giveaways_keyboard())
        await state.update_data(last_bot_message_id=sent.message_id)
        return
...
async def set_giveaway_duration(message: types.Message, state: FSMContext) -> None:
    ...
    try:
        ...
        if days < 0 or hours < 0 or minutes < 0:
            sent = await message.answer("Дни, часы и минуты не могут быть меньше 0. Попробуйте снова.", reply_markup=back_to_giveaways_keyboard())
            await state.update_data(last_bot_message_id=sent.message_id)
            return
        ...
        async with SessionLocal() as session:
            ...
            print(f"giveaway {giveaway.id} was scheduled at {giveaway.end_time}")

        sent = await message.answer("✅ Розыгрыш успешно создан!", reply_markup=back_to_giveaways_keyboard())
        await state.clear()
    except Exception as e:
        print(str(e))
        sent = await message.answer("❗ Введите 2 или 3 числа через пробел (дни часы [минуты]), например: <code>2 3</code> или <code>1 5 30</code>", parse_mode="HTML", reply_markup=back_to_giveaways_keyboard())
        await state.update_data(last_bot_message_id=sent.message_id)
        return
