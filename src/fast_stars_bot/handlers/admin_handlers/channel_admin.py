...
from keyboards.admin_keyboards import (delete_channels_keyboard,
                                      manage_channels_keyboard, channel_info_keyboard, back_to_channels_keyboard)
...
async def manage_channels_callback(callback: types.CallbackQuery, state: FSMContext) -> None:
    await state.clear()
...
async def add_channel_callback(callback: types.CallbackQuery, state: FSMContext) -> None:
    sent = await callback.message.edit_text("Введите название канала:", reply_markup=back_to_channels_keyboard())
...
async def add_channel_name(message: types.Message, state: FSMContext) -> None:
    ...
    sent = await message.answer("Введите @username или chat id:", reply_markup=back_to_channels_keyboard())
    await state.update_data(last_bot_message_id=sent.message_id)
...
async def add_channel_username(message: types.Message, state: FSMContext) -> None:
    ...
    sent = await message.answer("Введите ссылку на канал:", reply_markup=back_to_channels_keyboard())
    await state.update_data(last_bot_message_id=sent.message_id)
