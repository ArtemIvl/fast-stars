...
async def get_all_tasks(session: AsyncSession) -> list[Task]:
    result = await session.execute(select(Task).order_by(desc(Task.id)))
...
async def delete_task(session: AsyncSession, task_id: int) -> None:
    ...
    if task:
        await session.execute(
            delete(TaskCompletion).where(TaskCompletion.task_id == task_id)
        )
...
async def is_task_completed(session: AsyncSession, user_id: int, task_id: int) -> bool:
    result = await session.execute(
        select(TaskCompletion).where(
            TaskCompletion.user_id == user_id, TaskCompletion.task_id == task_id
        )
    )
    return result.scalars().first() is not None


async def get_tasks_by_ids(session: AsyncSession, task_ids: list[int]) -> list[Task]:
    if not task_ids:
        return []
