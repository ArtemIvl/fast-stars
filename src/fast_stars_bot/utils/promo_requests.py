...
async def is_promo_code_unique(session: AsyncSession, code: str) -> bool:
    result = await session.execute(select(PromoCode).filter(PromoCode.code == code))
...
async def activate_promo(
    session: AsyncSession, telegram_id: int, code: str, bot: Bot
...
async def get_promo_activations_page(
    session: AsyncSession, promo_code_id: int, page: int = 1, per_page: int = 10
