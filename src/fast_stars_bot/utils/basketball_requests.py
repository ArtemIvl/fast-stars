...
async def get_basketball_stats(
    session: AsyncSession, multiplier: Decimal
) -> tuple[int, Decimal, Decimal]:
    total_games = await session.scalar(select(func.count()).select_from(BasketballLog))

    total_won_stars = await session.scalar(
        select(
            func.coalesce(func.sum(BasketballLog.bet * multiplier), Decimal("0"))
