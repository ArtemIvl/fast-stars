...
from sqlalchemy import desc, select, func, distinct
...
def calculate_bonus_amount(last_claim: DailyBonusClaim | None, is_vip: bool) -> tuple[Decimal, int]:
    today = date.today()
...
async def claim_daily_bonus(session: AsyncSession, user: User) -> Decimal:
    ...
    claim = DailyBonusClaim(
        user_id=user.id, claim_date=date.today(), bonus_amount=bonus_amount, streak=streak
    )
    session.add(claim)
...
async def bonus_claims_today(session: AsyncSession) -> int:
    today = date.today()
    result = await session.execute(
        select(func.count(distinct(DailyBonusClaim.user_id)))
        .where(DailyBonusClaim.claim_date == today)
    )
    return result.scalar_one()

async def total_bonus_amount_claimed(session: AsyncSession) -> Decimal:
    result = await session.execute(
        select(func.sum(DailyBonusClaim.bonus_amount))
    )
