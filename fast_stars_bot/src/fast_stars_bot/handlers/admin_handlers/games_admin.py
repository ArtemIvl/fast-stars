...
from keyboards.admin_keyboards import admin_games_keyboard
...
async def manage_games_callback(callback: types.CallbackQuery) -> None:
    async with SessionLocal() as session:
        ...
        await callback.message.edit_text(text, parse_mode="HTML", reply_markup=admin_games_keyboard())
...
async def start_setting_change(callback: types.CallbackQuery, state: FSMContext):
    ...
    field_names = {
        ...
        "basketball_multiplier": "множитель победы в баскетболе (например, 1.6)",
        "cube_commission": "комиссию в кубиках (%)"
    }
    await callback.message.edit_text(f"Введите новое значение для: <b>{field_names[key]}</b>", parse_mode="HTML")
