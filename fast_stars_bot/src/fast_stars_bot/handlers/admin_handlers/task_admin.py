...
from keyboards.admin_keyboards import manage_tasks_keyboard, delete_tasks_keyboard, task_info_keyboard
...
async def manage_tasks_callback(callback: types.CallbackQuery) -> None:
    async with SessionLocal() as session:
        tasks = await get_all_tasks(session)
        await callback.message.edit_text(
            "Управление задачами:", reply_markup=manage_tasks_keyboard(tasks)
        )
...
async def add_task_title(message: types.Message, state: FSMContext) -> None:
    ...
    await message.answer("Введите URL задания:")
...
async def add_task_title(message: types.Message, state: FSMContext) -> None:
    ...
    await message.answer("Введите @username или chat_id:")
...
async def add_task_url(message: types.Message, state: FSMContext) -> None:
    ...
    await message.answer("Введите вознаграждение за выполнение задания (в звёздах ⭐):")
...
async def add_task_reward(message: types.Message, state: FSMContext) -> None:
    try:
        ...
        if reward <= 0:
            await message.answer("Вознаграждение должно быть больше 0. Попробуйте снова.")
        await state.update_data(task_reward=reward)
        await message.answer("Задание требует подписки? (да/нет):")
...
async def delete_task_handler(callback: types.CallbackQuery) -> None:
    ...
    async with SessionLocal() as session:
        ...
        await callback.message.edit_text(
            "Задание удалено!",
            reply_markup=manage_tasks_keyboard(await get_all_tasks(session)),
        )
